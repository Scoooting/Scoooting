...
class RentalServiceApplicationTests {
    ...
    void startAndEndRental() {
        RentalResponseDTO responseDTO = rentalService.startRental(1L, 1L, lat, lon);
        ...
        RentalResponseDTO rentalEnd = rentalService.endRental(responseDTO.userId(), lat + 0.5, lon + 0.5);
        assertAll(
    ...
    void startAndCancelRental() {
        RentalResponseDTO responseDTO = rentalService.startRental(1L, 1L, lat, lon);
        rentalService.cancelRental(responseDTO.userId());
        Rental rental = rentalRepository.findById(responseDTO.id()).orElseThrow();
